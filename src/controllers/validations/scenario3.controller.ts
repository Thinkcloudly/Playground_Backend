...
import { S3Client, AbortMultipartUploadCommand, GetBucketPolicyCommand, GetBucketPolicyCommandInput } from "@aws-sdk/client-s3";
...
router.post('/', async (req: Request, res: Response) => {
    ...
    try {
        ...
        let resId = Resources.find(x => x.ResourceType === "AWS::S3::Bucket")
        ...
        let s3PolicyInput: GetBucketPolicyCommandInput = {
            Bucket: resId.PhysicalResourceId
        }
        const s3Command = new GetBucketPolicyCommand(s3PolicyInput);
        let s3Resp = await s3Client.send(s3Command);
        console.log(s3Resp);
        if (s3Resp.Policy !== undefined) {
            return res.send({
                status: "success",
